...
export default withApiAuthRequired(async function handler(req, res) {
  ...
  const htmlContent = metaDescriptionResult.data.choices[0]?.message?.content;

  // Regular expression to match the meta description tag and its content
  const metaDescriptionRegex =
    /<meta\s+name="description"\s+content="([^"]*)"/i;
  const match = htmlContent.match(metaDescriptionRegex);
